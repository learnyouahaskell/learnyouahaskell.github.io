...
Having your whole program be just one I/O action seems kind of limiting. That's
...
Now we're going to make a program that continuously reads a line and prints out
...
When dealing with I/O *do* blocks, we mostly use `return` either because we need
...
See how we can use recursion in I/O, just like we can use it in pure code. Just
...
This program looks like it should read a character and then check if it's a
...
`forM`{.label .function} (located in `Control.Monad`) is like `mapM`, only that
...
In this section, we learned the basics of input and output. We also found out
...
We'll save that program as `capslocker.hs` or something and compile it. And
...
Again, we get the same output as if we had run our program and put in the words
...
Let's also go over the program line by line! Our program is several I/O actions
...
With `putStr contents` we just print the contents out to the standard output and
...
Here's our previous piece of code, only it doesn't read it line by line but
...
We already did pretty much the same thing in the program that only deleted tasks
...
Another way is to use the `newStdGen`{.label .function} action, which splits our
...
The bytestring version of `:` is called `cons`{.label .function} It takes a byte
...
to call `ioeGetFileName` with `e` and then pattern match against the `Maybe`
value that it returned. Using *case* expressions is commonly used when you want
to pattern match against something without bringing in a new function.
...
several of them with `catch` and use different handlers for them, like so:
...
`launchRockets` isn't a parameter to `catch`, so whichever exceptions it might
throw will likely crash our program, unless `launchRockets` uses `catch`
internally to handle its own exceptions. Of course `toTry`, `thenTryThis` and
`launchRockets` are I/O actions that have been glued together using *do* syntax
and hypothetically defined somewhere else. This is kind of similar to
*try-catch* blocks of other languages, where you can surround your whole program
in a single *try-catch* or you can use a more fine-grained approach and use
different ones in different parts of your code to control what kind of error
handling happens where.

Now you know how to deal with I/O exceptions! Throwing exceptions from pure code
and dealing with them hasn't been covered here, mainly because, like we said,
Haskell offers much better ways to indicate errors than reverting to I/O to
catch them. Even when glueing together I/O actions that might fail, I prefer to
have their type be something like `IO (Either a b)`, meaning that they're normal
I/O actions but the result that they yield when performed is of type `Either a
b`, meaning it's either `Left a` or `Right b`.
